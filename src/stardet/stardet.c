...
int __check_PGM (char const * path){
    ...
    for (i = 0; c != '.'; i++){ // Offset to the file extension.
        c = path[i];
        if (c == '\0') return -1; // Reached end: invalid file type.
    }
    ...
    for (int row = 0; row < img->height; row++){
        for (int col = 0; col < img->width; col++){
            ...
            for (int currow = row-1; currow < row+2; currow++) for (int curcol = col-1; curcol < col+2; curcol++){
                ...
                colours[colour_index] += img->data[currow][curcol];
            }

            for (int i = 0; i < 3; i++) colours[i] /= colour_count[i];// Take the average of all colours in the pixels neighbourhood.
            buf[row*img->width + col] = RGB_to_mono(colours[0], colours[1], colours[2]); // Convert to monochrome.
        }
    }
